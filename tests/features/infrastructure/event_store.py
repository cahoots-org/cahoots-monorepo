...
class InMemoryEventStore:
    ...
    def append_events(
        self, stream_id: Any, events: List[Any], expected_version: Optional[int] = None
    ) -> None:
        ...
        if expected_version is not None:
            ...
            if current_version != expected_version:
                raise ValueError(
                    f"Concurrency error: expected version {expected_version}, got {current_version}"
                )
        ...
        for events in self._events.values():
            for event in events:
                if (
                    ...
                    and hasattr(event.metadata, "correlation_id")
...
class InMemoryViewStore:
    ...
    def apply_event(self, event: Any) -> None:
        ...
        aggregate_id = (
            ...
            else getattr(event, "entity_id", None)
        )
