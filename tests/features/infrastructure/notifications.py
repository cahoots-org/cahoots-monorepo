...
class MockEmailService:
    ...
    def send_verification_email(self, email, token):
        ...
        self.sent_emails.append({"type": "verification", "to": email, "token": token})

    def send_password_reset_email(self, email, token):
        """Send a password reset email"""
        self.sent_emails.append({"type": "password_reset", "to": email, "token": token})

    def send_notification_email(self, email, subject, body):
        """Send a notification email"""
        self.sent_emails.append(
            {"type": "notification", "to": email, "subject": subject, "body": body}
        )
