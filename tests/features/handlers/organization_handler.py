...
from ..views.team_views import TeamMembersView, TeamView
...
class OrganizationCreated:
    ...
    def __init__(
        self, event_id, timestamp, metadata, organization_id, name, description, created_by
    ):
...
class OrganizationNameUpdated:
    ...
    def __init__(
        self, event_id, timestamp, metadata, organization_id, new_name, reason, updated_by
    ):
        self.event_id = event_id
        self.timestamp = timestamp
        self.metadata = metadata
...
class TeamCreated:
    ...
    def __init__(
        self, event_id, timestamp, metadata, team_id, organization_id, name, description, created_by
    ):
        self.event_id = event_id
        self.timestamp = timestamp
        self.metadata = metadata
...
class OrganizationHandler:
    ...
    def handle_create_team(self, cmd):
        ...
        team_view = self._view_store.get_view(team_id, self.team_view_factory)
        members_view = self._view_store.get_view(team_id, self.team_members_view_factory)
    ...
    def handle_add_team_member(self, cmd):
        ...
        if cmd.created_by not in members_view.members:
            org_members_view = self._view_store.get_view(
                team_view.organization_id, self.org_members_view_factory
            )
    ...
    def handle_update_team_member_role(self, cmd):
        ...
        print(
            f"User {cmd.user_id} role updated from {members_view.members[cmd.user_id]['role']} to {cmd.new_role}"
        )
    ...
    def handle_remove_team_member(self, cmd):
        ...
        if members_view.members[cmd.user_id]["role"] == "lead":
            lead_count = sum(
                1 for member in members_view.members.values() if member["role"] == "lead"
            )
            if lead_count <= 1:
    ...
    def handle_archive_team(self, cmd):
        ...
        event = TeamArchived(
            ...
            timestamp=datetime.utcnow(),
            metadata=EventMetadata(
                schema_version=1, correlation_id=cmd.correlation_id, actor_id=cmd.archived_by
            ),
            team_id=cmd.team_id,
            reason=cmd.reason,
