...
class CreateProject(Command):
    ...
    def __init__(
        self, command_id, correlation_id, name, description, repository, tech_stack, created_by
    ):
...
class CompleteTask(Command):
    ...
    def __init__(
        self, command_id, correlation_id, project_id, requirement_id, task_id, completed_by
    ):
        super().__init__(command_id, correlation_id)
        self.project_id = project_id
        self.requirement_id = requirement_id
...
class SetProjectTimeline(Command):
    ...
    def __init__(
        self, command_id, correlation_id, project_id, start_date, target_date, milestones, set_by
    ):
        super().__init__(command_id, correlation_id)
        self.project_id = project_id
...
class UnblockRequirement(Command):
    ...
    def __init__(
        self, command_id, correlation_id, project_id, requirement_id, resolution, unblocked_by
    ):
        super().__init__(command_id, correlation_id)
        self.project_id = project_id
        self.requirement_id = requirement_id
...
def step_create_project(context: Context, project_name: str):
    ...
    project_view = context.view_store.get_view(context.current_project_id, ProjectOverviewView)
...
def step_complete_all_requirements(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
...
def step_set_timeline(context: Context, agent_id: str):
    row = context.table[0]
    milestones = [
        {"date": parse_date(row["milestone_date"]), "description": row["milestone_description"]}
    ]
...
def step_change_requirement_priority(context: Context, agent_id: str):
    ...
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = next(
        req for req in view.requirements.values() if req["title"] == row["requirement"]
    )
...
def step_change_task_priority(context: Context, agent_id: str):
    ...
    task = next(task for task in requirement["tasks"] if task["title"] == row["task"])
...
def step_check_project_created(context: Context):
    view = context.view_store.get_view(context.current_project_id, ProjectOverviewView)
...
def step_check_requirement_added(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    assert context.current_requirement_id in view.requirements, "Requirement not found in project"
...
def step_check_requirement_dependency(context: Context, req_name: str, dep_name: str):
    ...
    assert (
        dependency["id"] in requirement["dependencies"]
    ...
    if dependency["id"] in view.requirement_dependencies:
        assert (
            requirement["id"] in view.requirement_dependencies[dependency["id"]]
...
def step_check_requirements_order(context: Context):
    ...
    for req in view.requirements.values():
        for dep_id in req["dependencies"]:
            ...
            assert dep["id"] != req["id"], f"Requirement {req['title']} cannot depend on itself"
...
def step_check_requirement_task_count(context: Context, count: int):
    ...
    assert len(active_tasks) == count, f"Expected {count} active tasks, got {len(active_tasks)}"


@when('all tasks for requirement "{req_name}" are completed')
def step_complete_requirement_tasks(context: Context, req_name: str):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = next(req for req in view.requirements.values() if req["title"] == req_name)
    ...
    if hasattr(project_view, "active_requirement_ids"):
        ...
        print(
            f"Project overview - completed_requirement_ids: {project_view.completed_requirement_ids}"
        )
...
def step_check_requirement_completed(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    assert requirement["status"] == "completed", "Requirement not marked as completed"
...
def step_check_milestone(context: Context, description: str, date: str):
    view = context.view_store.get_view(context.current_project_id, ProjectOverviewView)
    milestone = next((m for m in view.milestones if m["description"] == description), None)
    assert milestone is not None, f"Milestone '{description}' not found"
    assert milestone["date"] == parse_date(
        date
...
def step_check_project_status(context: Context, status: str):
    view = context.view_store.get_view(context.current_project_id, ProjectOverviewView)
    assert view.status == status, f"Expected project status {status}, got {view.status}"
...
def step_check_requirement_blocked(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    assert requirement["status"] == "blocked", "Requirement not marked as blocked"
...
def step_check_requirement_blocker(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    assert requirement["blocker_description"] is not None, "No blocker description found"
...
def step_check_requirement_active(context: Context):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    assert requirement["status"] == "active", "Requirement not marked as active"
...
def step_check_requirement_priority(context: Context, title: str, priority: str):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = next(req for req in view.requirements.values() if req["title"] == title)
    assert (
...
def step_check_task_assigned(context: Context, assignee_id: str):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    task = next(task for task in requirement["tasks"] if task["id"] == context.last_task_id)
    assert task["assignee_id"] == get_agent_id(
        context, assignee_id
...
def step_check_task_blocked(context: Context):
    ...
    requirement = view.requirements[context.current_requirement_id]
    task = next(task for task in requirement["tasks"] if task["id"] == context.last_task_id)
    assert task["status"] == "blocked", "Task not marked as blocked"
...
def step_check_task_blocker(context: Context):
    ...
    requirement = view.requirements[context.current_requirement_id]
    task = next(task for task in requirement["tasks"] if task["id"] == context.last_task_id)
    assert task["blocker_description"] is not None, "No blocker description found"
...
def step_check_task_active(context: Context):
    ...
    requirement = view.requirements[context.current_requirement_id]
    task = next(task for task in requirement["tasks"] if task["id"] == context.last_task_id)
    assert task["status"] == "active", "Task not marked as active"
...
def step_check_task_priority(context: Context, title: str, priority: str):
    view = context.view_store.get_view(context.current_project_id, RequirementsView)
    requirement = view.requirements[context.current_requirement_id]
    task = next(task for task in requirement["tasks"] if task["title"] == title)
    ...
    try:
        ...
        if dependencies:
            ...
            if context.current_requirement_id in view.requirement_dependencies:
                print(
                    f"Actual dependencies in map: {view.requirement_dependencies[context.current_requirement_id]}"
                )
                assert (
                    view.requirement_dependencies[context.current_requirement_id] == dependencies
                ...
                print(
                    f"Warning: Requirement ID {context.current_requirement_id} not found in dependencies map"
                )
