...
from tests.features.steps.common import CommandBus, ensure_agent_id, get_agent_id
from tests.features.steps.common_steps import (
    step_check_error_message,
    step_system_user_exists,
)
from tests.features.views.organization_views import (
...
class UpdateOrganizationMemberRole:
    def __init__(
        self, command_id, correlation_id, organization_id, user_id, new_role, reason, changed_by
    ):
        self.command_id = command_id
        self.correlation_id = correlation_id
        self.organization_id = organization_id
...
class UpdateOrganizationProfile:
    def __init__(
        self, command_id, correlation_id, organization_id, new_name, new_description, updated_by
    ):
        self.command_id = command_id
        self.correlation_id = correlation_id
        self.organization_id = organization_id
...
def step_organization_exists(context: Context, org_name: str):
    ...
    assert (
        details_view.name == org_name
    ), f"Expected organization name '{org_name}' but got '{details_view.name}'"
    assert (
        user_id in members_view.roles["admin"]
    ), f"User {user_id} not in admin roles: {members_view.roles['admin']}"


@given('agent "{user_id}" is an admin of the organization')
def step_user_is_admin(context: Context, user_id: str):
    view = context.view_store.get_view(context.current_organization_id, OrganizationMembersView)
    assert get_agent_id(context, user_id) in view.roles["admin"], f"User {user_id} is not an admin"
...
def step_user_is_only_admin(context: Context, user_id: str):
    view = context.view_store.get_view(context.current_organization_id, OrganizationDetailsView)
    assert view.admin_count == 1, f"Expected 1 admin, found {view.admin_count}"
    members_view = context.view_store.get_view(
        context.current_organization_id, OrganizationMembersView
    )
    assert (
        get_agent_id(context, user_id) in members_view.roles["admin"]
...
def step_check_organization_details(context: Context):
    row = context.table[0]
    view = context.view_store.get_view(context.current_organization_id, OrganizationDetailsView)
    assert view.name == row["name"], f"Expected name {row['name']}, got {view.name}"
    assert view.member_count == int(
        row["member_count"]
...
def step_check_organization_name(context: Context, name: str):
    view = context.view_store.get_view(context.current_organization_id, OrganizationDetailsView)
    assert view.name == name, f"Expected name {name}, got {view.name}"


@then("the change should be recorded in the audit log")
def step_check_audit_log(context: Context):
    view = context.view_store.get_view(context.current_organization_id, OrganizationAuditLogView)
...
def step_check_member_count(context: Context, count: int):
    ...
    assert view.member_count == count, f"Expected {count} members, got {view.member_count}"
...
def step_check_user_role(context: Context, user_id: str, role: str):
    view = context.view_store.get_view(context.current_organization_id, OrganizationMembersView)
    assert get_agent_id(context, user_id) in view.roles[role], f"User {user_id} is not a {role}"
...
def step_check_user_not_member(context: Context, user_id: str):
    ...
    for role_members in view.roles.values():
        assert user_id not in role_members, f"User {user_id} is still a member"
...
def step_check_member_count_singular(context: Context, count: int):
    view = context.view_store.get_view(context.current_organization_id, OrganizationDetailsView)
    assert view.member_count == count, f"Expected {count} member, got {view.member_count}"
