...
class UpdateTeamMemberRole:
    def __init__(
        self, command_id, correlation_id, team_id, member_id, new_role, reason, updated_by
    ):
        self.command_id = command_id
        self.correlation_id = correlation_id
        self.team_id = team_id
...
def step_check_team_created(context: Context):
    view = context.view_store.get_view(context.current_team_id, TeamView)
...
def step_check_team_details(context: Context):
    row = context.table[0]
    view = context.view_store.get_view(context.current_team_id, TeamView)
    assert view.name == row["name"], f"Expected team name {row['name']}, got {view.name}"
    assert len(view.members) == int(
        row["member_count"]
...
def step_check_team_members(context: Context, count: int):
    view = context.view_store.get_view(context.current_team_id, TeamView)
    assert len(view.members) == count, f"Expected {count} members, got {len(view.members)}"
...
def step_check_member_role(context: Context, agent_id: str, role: str):
    ...
    assert (
        view.members[member_id]["role"] == role
...
def step_member_is_lead(context: Context, agent_id: str):
    ...
    team_view = context.view_store.get_view(context.current_team_id, TeamView)
    current_lead = None
...
def step_check_member_removed(context: Context, agent_id: str):
    view = context.view_store.get_view(context.current_team_id, TeamView)
    member_id = ensure_agent_id(context, agent_id)
    assert member_id not in view.members, f"Member {agent_id} still in team"
...
def step_check_members_notified(context: Context):
    ...
    assert view.last_notification["type"] == "team_archived", "Wrong notification type"
