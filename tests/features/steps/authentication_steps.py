...
from tests.features.test_imports import Event, EventMetadata, Login
from tests.features.test_imports import Logout as LogoutUser
from tests.features.test_imports import RefreshAccessToken as RefreshToken
...
def setup_mock_services(context):
    ...
    if not hasattr(context, "auth_handler"):
        ...
        if hasattr(context, "view_store"):
            print(
                "ViewStore methods:",
                [method for method in dir(context.view_store) if not method.startswith("_")],
            )
...
def step_check_user_registered_successfully(context: Context):
    ...
    assert (
        context.users[context.current_user_email]["email"] == context.current_user_email
    ), "User email was not set correctly"
...
def step_login_with_valid_credentials(context: Context):
    ...
    cmd = Login(
        command_id=str(uuid4()), correlation_id=str(uuid4()), email=email, password=password
    )
...
def step_check_invalid_credentials_error(context: Context):
    ...
    assert (
        "Invalid credentials" in context.error_message or "invalid" in context.error_message.lower()
    ), f"Expected 'Invalid credentials' error, got: {context.error_message}"
...
def step_request_password_reset(context: Context):
    ...
    cmd = RequestPasswordReset(command_id=str(uuid4()), correlation_id=str(uuid4()), email=email)
    ...
    try:
        ...
        context.email_service.send_email(
            email, "Reset Your Password", f"Click here to reset your password: {reset_url}"
        )
...
def step_check_refresh_token_remains_valid(context: Context):
    ...
    if hasattr(context, "sessions") and email in context.sessions:
        ...
        if isinstance(context.sessions[email], dict) and any(
            isinstance(v, dict) for v in context.sessions[email].values()
        ):
            ...
            for device, session in context.sessions[email].items():
                if isinstance(session, dict) and "refresh_token" in session:
                    ...
                    ), f"Refresh token for device {device} is not valid"
        # Else it might be a simple session dictionary
        elif (
            isinstance(context.sessions[email], dict) and "refresh_token" in context.sessions[email]
        ):
            assert (
                context.sessions[email]["refresh_token"] is not None
            ), "Refresh token in sessions dict is not valid"
...
def step_check_session_invalidated(context: Context):
    ...
    assert not context.revoked_session[
        "is_active"
    ], f"Session {context.revoked_session['session_id']} was not revoked"
    ...
    if user_obj and hasattr(user_obj, "get_active_sessions"):
        ...
        ), f"Session {session_id} is still active in user object"
    ...
    if hasattr(context, "sessions") and email in context.sessions:
        for device, session in context.sessions[email].items():
            if "is_active" not in session:
                ...
                )  # All except revoked should be active
...
def step_check_new_access_token(context: Context):
    ...
    assert (
        context.current_session["access_token"] != context.current_session["old_access_token"]
    ), "New access token is identical to old one"
...
def step_user_logout(context: Context):
    ...
    cmd = LogoutUser(
        command_id=str(uuid4()), correlation_id=str(uuid4()), user_id=user_id, session_id=session_id
    )
...
def step_check_login_rejected(context: Context):
    ...
    assert (
        hasattr(context, "login_succeeded") and context.login_succeeded is False
    ), "Login should have been rejected"
...
def step_check_password_reset_email_sent(context: Context):
    ...
    for sent_email in context.email_service.sent_emails:
        if sent_email["to"] == email and (
            "reset" in sent_email["subject"].lower() or "password" in sent_email["subject"].lower()
        ):
            found_email = True
            break
...
def step_check_password_updated(context: Context):
    ...
    assert (
        hasattr(context, "password_updated") and context.password_updated
    ), "Password was not updated"
    assert (
        not hasattr(context, "exception") or context.exception is None
    ), f"Password reset failed with error: {getattr(context, 'exception', None)}"
...
def step_check_login_with_new_password(context: Context):
    ...
    # Create login command
    cmd = Login(
        command_id=str(uuid4()), correlation_id=str(uuid4()), email=email, password=new_password
    )
