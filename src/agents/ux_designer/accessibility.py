    ...
    def _check_color_contrast(self, layout: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if ratio < 4.5:  # WCAG AA standard for normal text
                results["valid"] = False
                results["violations"].append(f"low-contrast: Insufficient contrast ratio ({ratio:.2f}:1)")
                results["recommendations"].append(f"increase-contrast: Increase ratio to at least 4.5:1")
    ...
    def _check_text_sizes(self, layout: Dict[str, Any], viewport: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if viewport == "mobile" and font_size < 16:
                results["warnings"].append(f"mobile-text: Text size too small for mobile ({font_size}px)")
        ...
        try:
            ...
            if layout.get("role") in ["button", "link", "checkbox", "radio"]:
                ...
                for attr in required_attrs.get(layout["role"], []):
                    if attr not in layout.get("aria", {}):
                        results["valid"] = False
                        results["violations"].append(f"missing-aria: Missing {attr} for {layout['role']}")
            ...
            if layout.get("interactive", False):
                ...
                if width < 44 or height < 44:
                    results["warnings"].append(f"small-target: Touch target below 44x44px ({width}x{height}px)")
