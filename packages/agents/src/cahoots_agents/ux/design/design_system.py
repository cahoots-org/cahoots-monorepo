...
class DesignSystem:
    ...
    def normalize_typography(self, typography: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for key, value in styles.items():
            ...
            if any(color_prop in normalized_key for color_prop in ['color', 'background', 'border', 'shadow']):
                if isinstance(value, str):
                    normalized[normalized_key] = next(iter(self.normalize_colors({'temp': value}).values()))
                else:
