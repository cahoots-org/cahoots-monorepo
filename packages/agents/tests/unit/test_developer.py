...
import uuid
import asyncio
from asyncio import TaskGroup  # Add TaskGroup import at top

from packages.agent_developer import Developer
...
@pytest.mark.asyncio
async def test_needs_ux_design_comprehensive(
    developer: Developer,
) -> None:
    """Test comprehensive UX design requirement detection."""
    # Test explicit requires_ux flag
    ux_task = Task(
        id=str(uuid.uuid4()),
        title="Regular Task",
        description="A task with requires_ux=True",
        requires_ux=True,
        metadata={}
    )
    assert developer.needs_ux_design([ux_task])

    # Test required skills detection
    skill_task = Task(
        id=str(uuid.uuid4()),
        title="UI Task",
        description="A task requiring UI skills",
        requires_ux=False,
        metadata={"required_skills": ["python", "ui", "react"]}
    )
    assert developer.needs_ux_design([skill_task])

    # Test non-UX task
    regular_task = Task(
        id=str(uuid.uuid4()),
        title="Backend Task",
        description="A pure backend task",
        requires_ux=False,
        metadata={"required_skills": ["python", "database"]}
    )
    assert not developer.needs_ux_design([regular_task])

    # Test mixed task list
    assert developer.needs_ux_design([regular_task, ux_task])
    assert developer.needs_ux_design([regular_task, skill_task])
    assert not developer.needs_ux_design([regular_task])

@pytest.mark.asyncio
async def test_implement_tasks_error_handling(
    developer: Developer,
    mock_tasks: List[Task],
    caplog: "LogCaptureFixture"
) -> None:
    """Test comprehensive error handling during task implementation."""
    ...
    for error in errors:
        developer.code_generator.generate_implementation.side_effect = error
        result = await developer.implement_tasks([mock_tasks[0]])
        
        # Verify error handling
        assert len(result["implementations"]) == 0
        assert len(result["failed_tasks"]) == 1
        failed_task = result["failed_tasks"][0]
        assert failed_task["task_id"] == mock_tasks[0].id
        assert str(error) in failed_task["error"]
        assert str(error) in caplog.text

    # Test mixed success/failure
    developer.code_generator.generate_implementation.side_effect = [
        {"code": "def test(): pass", "file_path": "test.py"},
        ValueError("Second task failed")
    ]
    
    result = await developer.implement_tasks(mock_tasks[:2])
    assert len(result["implementations"]) == 1
    assert len(result["failed_tasks"]) == 1
    assert mock_tasks[0].id in result["implementations"]
    assert any(t["task_id"] == mock_tasks[1].id for t in result["failed_tasks"])
