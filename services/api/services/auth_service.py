...
class AuthService:
    ...
    async def refresh_tokens(self, refresh_token: str) -> Tuple[str, str]:
        ...
        try:
            ...
            access_token, new_refresh_token = await self.security_manager.create_tokens(
                payload["sub"]
            )
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error refreshing tokens"
            )
    ...
    async def reset_password(self, token: str, new_password: str):
        ...
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Invalid or expired reset token"
            )
    ...
    async def verify_email(self, token: str) -> User:
        ...
        stmt = select(User).where(User.verification_token == token, User.is_verified == False)
        result = await self.db.execute(stmt)
        user = result.scalar_one_or_none()
