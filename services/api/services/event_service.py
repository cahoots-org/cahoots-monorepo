...
class EventService:
    ...
    def __init__(self, config: EventConfig, redis: Redis, db: Any):
    ...
    async def _acquire_cleanup_lock(self) -> bool:
        ...
        return await self.redis.set(self.CLEANUP_LOCK_KEY, "1", ex=self.LOCK_TIMEOUT, nx=True)
    ...
    async def save_event(self, event: Event) -> None:
        ...
        try:
            ...
            await self.redis.setex(f"event:{event.id}", self.config.cache_ttl_seconds, event_json)
    ...
    async def get_event(self, event_id: UUID) -> Optional[Event]:
        ...
        if event:
            # Update cache
            event_json = event.model_dump_json()
            await self.redis.setex(cache_key, self.config.cache_ttl_seconds, event_json)
        return event

    async def get_project_events(
        self, project_id: UUID, status: Optional[EventStatus] = None
    ) -> List[Event]:
