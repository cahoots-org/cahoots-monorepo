...
class ContextEventBus:
    ...
    def __init__(
        self, db: Session, redis: Optional[Redis] = None, channel_prefix: str = "context_events"
    ):
    ...
    async def publish(self, project_id: UUID, event_type: str, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise SystemError(
                ...
                original_error=e,
                context={"project_id": str(project_id), "event_type": event_type},
            )

    async def subscribe(self, project_id: UUID, callback: Callable[[Dict[str, Any]], None]) -> None:
    ...
    async def unsubscribe(
        self, project_id: UUID, callback: Callable[[Dict[str, Any]], None]
    ) -> None:
