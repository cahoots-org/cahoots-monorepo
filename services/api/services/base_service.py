...
from utils.metrics import Counter as CIRCUIT_BREAKER_FAILURES
from utils.metrics import Counter as SERVICE_ERROR_COUNTER
from utils.metrics import Counter as SERVICE_REQUEST_COUNTER
from utils.metrics import Gauge as CIRCUIT_BREAKER_STATE
from utils.metrics import Histogram as SERVICE_REQUEST_TIME
...
class CircuitBreakerState:
    ...
    def __init__(
        self, failure_threshold: int = 5, reset_timeout: int = 60, test_request_limit: int = 1
    ):
    ...
    async def get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            self._client = httpx.AsyncClient(
                timeout=self.config.timeout, verify=True  # Enable SSL verification
            )
    ...
    async def _make_request(
        ...
        if not should_allow:
            ...
            if self.deps and self.deps.event_system:
                await self.deps.event_system.emit(
            ...
            return ServiceResponse(success=False, error="Circuit breaker is open", status_code=503)
        ...
        self.logger.info(
            f"Entering _make_request with method: {method}, endpoint: {endpoint}, headers: {headers}"
        )
        ...
        while retry_count <= self.config.retry_attempts:
            try:
                if retry_count > 0:
                    ...
                    self.logger.info(
                        f"Retrying request after {delay:.2f}s delay (attempt {retry_count})"
                    )
                    await asyncio.sleep(delay)

                SERVICE_REQUEST_COUNTER.labels(
                    service=self.config.name, method=method, endpoint=endpoint
                ).inc()

                response = await client.request(
                    method=method, url=f"{self.config.url}{endpoint}", json=data, headers=headers
                )
                ...
                if response and response.status_code >= 500:
                    ...
                    CIRCUIT_BREAKER_FAILURES.labels(
                        service=self.config.name, error_type="http_5xx"
                    ).inc()
                    if self.deps and self.deps.event_system:
                        await self.deps.event_system.emit(
                ...
                SERVICE_ERROR_COUNTER.labels(
                    service=self.config.name, error_type="http_error"
                ).inc()
                ...
                await self.circuit_breaker.record_failure()
                CIRCUIT_BREAKER_FAILURES.labels(
                    service=self.config.name, error_type="unexpected"
                ).inc()
                if self.deps and self.deps.event_system:
                    await self.deps.event_system.emit(
                        "circuit_breaker.failure",
                ...
                retry_count += 1
                SERVICE_ERROR_COUNTER.labels(
                    service=self.config.name, error_type="unexpected_error"
                ).inc()
        ...
        return ServiceResponse(
            success=False, error=f"Max retries exceeded. Last error: {last_error}", status_code=500
        )
