...
class EmailService:
    ...
    async def send_verification_email(self, email: EmailStr, token: str):
        ...
        message = MessageSchema(
            subject="Verify Your Email Address", recipients=[email], body=html, subtype="html"
        )
    ...
    async def send_password_reset_email(self, email: EmailStr, token: str):
        ...
        message = MessageSchema(
            subject="Reset Your Password", recipients=[email], body=html, subtype="html"
        )
    ...
    async def send_welcome_email(self, email: EmailStr, name: str):
        ...
        message = MessageSchema(
            subject="Welcome to Cahoots!", recipients=[email], body=html, subtype="html"
        )

        await self.fastmail.send_message(message)

    async def send_project_resources_email(self, email: EmailStr, project_data: Dict[str, Any]):
