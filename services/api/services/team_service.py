...
from cahoots_core.models.team_config import (
    ProjectLimits,
    RoleConfig,
    ServiceRole,
    TeamConfig,
...
class TeamService:
            ...
            if not roles:
                roles = {
                    ...
                    "member": RoleConfig(
                        ...
                        permissions={"manage_projects": True, "manage_members": False},
                    ),
                    "viewer": RoleConfig(role=ServiceRole.VIEWER, permissions={"view_only": True}),
                }
    ...
    async def get_team(self, team_id: str) -> Optional[TeamConfig]:
        ...
        try:
            ...
            if team:
                # Cache the result
                await self.deps.redis.set(f"team:{team_id}", team.model_dump_json(), expire=3600)
            return team
        except Exception as e:
    ...
    async def update_team_config(self, config: TeamConfig) -> TeamConfig:
        ...
        try:
            ...
            if not self._validate_limits(config.limits, project_limits):
                raise HTTPException(
                    status_code=400, detail="Team configuration exceeds project limits"
                )
            ...
            await self.deps.redis.set(
                f"team:{config.team_id}", updated.model_dump_json(), expire=3600
            )
    ...
    async def update_role_config(
        self, role: ServiceRole, config: RoleConfig, team_id: str
    ) -> TeamConfig:
    ...
    async def scale_role_instances(self, role: ServiceRole, instances: int) -> None:
        ...
        try:
            ...
            # Update replicas
            await self.deps.k8s.scale_deployment(deployment.metadata.name, instances)
        except Exception as e:
            logger.error(f"Failed to scale role instances: {e}")
            raise

    def _validate_limits(self, team_limits: ProjectLimits, project_limits: ProjectLimits) -> bool:
        """Validate team limits against project limits."""
        return (
            team_limits.max_projects <= project_limits.max_projects
            and team_limits.max_users <= project_limits.max_users
            and team_limits.max_storage_gb <= project_limits.max_storage_gb
            and team_limits.max_compute_units <= project_limits.max_compute_units
        )
    ...
    async def _get_project_limits(self) -> ProjectLimits:
        ...
        return ProjectLimits(
            max_projects=100, max_users=500, max_storage_gb=1000, max_compute_units=10000
        )
