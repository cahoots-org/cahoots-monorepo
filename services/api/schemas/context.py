...
class ContextEventCreate(BaseModel):
    ...
    event_type: str = Field(..., description="Type of context event (code_change, architectural_decision, standard_update)")
    event_data: Dict[str, Any] = Field(..., description="Event data payload")
    version_vector: Dict[str, int] = Field(default={"master": 0}, description="Version vector for optimistic concurrency control")
...
class VersionVectorResponse(BaseModel):
    ...
    version_vector: Dict[str, int] = Field(..., description="Current version vector")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of the version vector") 