...
class SubscriptionValidator:
    ...
    def __init__(self, subscription_service: Optional[SubscriptionService] = None):
        ...
        self.tier_limits = {
            "basic": {
                ...
                "resource_limits": {"cpu": "2", "memory": "4Gi", "pods": "5"},
            },
            "premium": {
                "projects": 10,
                "agents_per_project": 5,
                "resource_limits": {"cpu": "8", "memory": "16Gi", "pods": "20"},
            },
            "enterprise": {
                "projects": -1,  # Unlimited
                "agents_per_project": -1,  # Unlimited
                "resource_limits": {"cpu": "32", "memory": "64Gi", "pods": "100"},
            },
        }

    async def validate_limits(self, organization_id: str, metric: str, value: int) -> bool:
    ...
    async def validate_resources(self, organization_id: str, resources: Dict) -> bool:
    ...
    async def __call__(self, request: Request):
        ...
        if request.url.path.endswith("/projects") and request.method == "POST":
            ...
            if not await self.validate_limits(org_id, "projects", project_count + 1):
                raise HTTPException(
                    status_code=402, detail="Project limit reached for subscription tier"
                )
        ...
        if "agents" in request.url.path and request.method == "POST":
            ...
            if project_id:
                ...
                if not await self.validate_limits(org_id, "agents_per_project", agent_count + 1):
                    raise HTTPException(
                        status_code=402, detail="Agent limit reached for subscription tier"
                    )
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            ...
            if "resource_limits" in body:
                if not await self.validate_resources(org_id, body["resource_limits"]):
                    raise HTTPException(
                        status_code=402, detail="Resource limits exceed subscription tier"
                    )
