...
from api.dependencies import get_db, get_security_config, get_security_manager
...
async def social_auth(
    ...
    logger.info(
        "[OAUTH_FLOW] Received social auth request:",
        extra={
            "provider": provider,
    ...
    logger.info("[OAUTH_FLOW] Full user data:", extra={"user_data": user_data.model_dump()})
    ...
    try:
        ...
        logger.info("[OAUTH_FLOW] Converted user data:", extra={"user_dict": user_dict})

        # Create/update user and generate tokens
        user, access_token, refresh_token = await auth_service.handle_social_user(
            provider=provider, user_data=user_dict
        )

        logger.info(
            "[OAUTH_FLOW] Social authentication successful:",
            extra={
                "user_id": str(user.id),
                "email": user.email,
        ...
        logger.error(
            "[OAUTH_FLOW] Social authentication failed:",
            extra={
                "error": str(e),
