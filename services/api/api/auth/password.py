...
async def forgot_password(
    ...
    try:
        user, reset_token = await auth_service.create_password_reset(email=request.email)

        await email_service.send_password_reset_email(email=user.email, token=reset_token)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
...
async def reset_password(request: PasswordReset, db: AsyncSession = Depends(get_db)) -> None:
