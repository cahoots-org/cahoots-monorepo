...
from schemas.auth import EmailVerificationRequest, ResendVerificationRequest
...
async def verify_email(
    ...
    try:
        ...
        # Send welcome email
        await email_service.send_welcome_email(email=user.email, name=user.full_name)
...
async def resend_verification(
    ...
    try:
        ...
        await email_service.send_verification_email(email=user.email, token=token)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
