...
async def append_event(project_id: UUID, event: ContextEventCreate, db: Session = Depends(get_db)):
    """
...
async def get_context(project_id: UUID, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        return ContextResponse(context=context, version_vector=vector.versions)
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_version_vector(project_id: UUID, db: Session = Depends(get_db)):
    """
    ...
    try:
        vector = await context_service.get_version_vector(project_id)
        return VersionVectorResponse(version_vector=vector.versions, timestamp=vector.timestamp)
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
