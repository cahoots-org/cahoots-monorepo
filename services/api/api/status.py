...
from api.dependencies import BaseDeps
...
async def get_system_status(deps: BaseDeps = Depends()) -> SystemStatusResponse:
    ...
    try:
        ...
        return SystemStatusResponse(
            services=services, metrics=metrics, project_metrics=project_metrics
        )
...
async def _check_api_health() -> ServiceHealth:
    """Check API health."""
    return ServiceHealth(status="healthy", message="API is responding")
...
async def _check_db_health(db: AsyncSession) -> ServiceHealth:
    ...
    try:
        ...
        return ServiceHealth(status="healthy", message="Database is connected")
    except Exception as e:
        logger.error("Database health check failed: %s", str(e), exc_info=True)
        return ServiceHealth(status="unhealthy", message=f"Database error: {str(e)}")
...
async def _check_redis_health(redis: Redis) -> ServiceHealth:
    ...
    try:
        ...
        return ServiceHealth(
            status="healthy", message=f"Redis is connected (Memory: {used_memory})"
        )
    except Exception as e:
        logger.error("Redis health check failed: %s", str(e), exc_info=True)
        return ServiceHealth(status="unhealthy", message=f"Redis error: {str(e)}")
...
async def _check_event_system_health(event_system) -> ServiceHealth:
    ...
    try:
        ...
        return ServiceHealth(status="healthy", message="Event system is connected")
    except Exception as e:
        logger.error("Event system health check failed: %s", str(e), exc_info=True)
        return ServiceHealth(status="unhealthy", message=f"Event system error: {str(e)}")
...
async def _get_agent_health(redis: Redis) -> Dict[str, ServiceHealth]:
    ...
    for agent_type in agent_types:
        try:
            ...
            if last_heartbeat:
                agents[agent_type] = ServiceHealth(status="healthy", message="Agent is running")
            else:
                agents[agent_type] = ServiceHealth(
                    status="unhealthy", message="No recent heartbeat"
                )
        except Exception as e:
            logger.error("Agent health check failed for %s: %s", agent_type, str(e), exc_info=True)
            agents[agent_type] = ServiceHealth(
                status="unknown", message=f"Error checking agent: {str(e)}"
            )
...
async def _get_project_metrics() -> Optional[ProjectMetrics]:
    ...
    try:
        ...
        return ProjectMetrics(
            active_projects=int(active), completed_tasks=int(completed), success_rate=success
        )
    except Exception as e:
