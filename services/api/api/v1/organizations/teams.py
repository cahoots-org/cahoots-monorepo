...
from schemas.organizations import TeamAssignment, TeamResponse
...
async def assign_team(
    ...
    with metrics.timer("assign_team_duration"):
        try:
            ...
            result = await service.assign_team(organization_id, assignment.team_id, current_user.id)
...
async def remove_team(
    ...
    with metrics.timer("remove_team_duration"):
        try:
            service = OrganizationService(db)
            result = await service.remove_team(organization_id, team_id, current_user.id)
