...
from schemas.health import DependencyCheckResponse, DependencyDetails, DependencyStatus
...
async def get_dependency_details(
    dependency_name: str, db: AsyncSession = Depends(get_db)
...
async def verify_dependency(
    ...
    try:
        ...
        return APIResponse(success=True, data=is_verified)
    except Exception as e:
        return APIResponse(
            success=False,
            error=ErrorDetail(
