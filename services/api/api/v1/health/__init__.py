...
async def get_health_status(db: AsyncSession = Depends(get_db)) -> APIResponse[HealthStatus]:
...
async def readiness_check(db: AsyncSession = Depends(get_db)) -> APIResponse[bool]:
    """Check if the system is ready to handle requests."""
    try:
        service = HealthService(db)
        is_ready = await service.check_readiness()

        return APIResponse(success=True, data=is_ready)
    except Exception as e:
        return APIResponse(
            success=False,
            error=ErrorDetail(
