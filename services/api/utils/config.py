...
class ServiceConfig(BaseSettings):
    ...
    public_url: str = Field(
        default="http://localhost:3000", description="Public URL for the application"
    )
    ...
    auth_token_expire_minutes: int = Field(
        default=30, description="Auth token expiration in minutes"
    )
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/cahoots",
    ...
    k8s_config_path: Optional[str] = Field(
        default=None, description="Path to Kubernetes config file. If None, uses in-cluster config."
    )
    k8s_namespace: str = Field(default="cahoots", description="Default Kubernetes namespace")
    ...
    # Service
    service_name: str = Field(default="cahoots-service", description="Service name")
    ...
    default_resource_limits: Dict = Field(
        default={"cpu": "100m", "memory": "128Mi"}, description="Default resource limits"
    )
