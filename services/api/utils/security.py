...
from libs.core.cahoots_core.utils.infrastructure.redis.client import (
    RedisClient,
    RedisConfig,
)
...
class PermissionLevel(str, Enum):
    ...
    def __ge__(self, other: "PermissionLevel") -> bool:
        ...
        levels = {self.READ: 1, self.WRITE: 2, self.ADMIN: 3}
...
class RoleManager:
    ...
    async def check_permission(self, role_name: str, required_permission: Permission) -> bool:
        ...
        for permission in role_permissions:
            ...
            if (
                ...
                and required_permission.resource_id
                and permission.resource_id != required_permission.resource_id
            ):
                continue
    ...
    async def validate_key_usage(self, key_name: str, data: Dict[str, Any]) -> bool:
        ...
        for policy in policies:
            if not self._validate_against_policy(policy, data):
                self.logger.warning(
                    f"Key {key_name} failed validation against policy {policy.name}"
                )
                return False
...
class TokenManager:
    ...
    async def create_access_token(
        ...
        token = jwt.encode(to_encode, self.config.jwt_secret, algorithm=self.config.jwt_algorithm)
    ...
    async def create_session(
        self, session_id: str, data: Dict[str, Any], expires_in: timedelta
    ) -> None:
        ...
        await self.redis.setex(f"session:{session_id}", expires_in.total_seconds(), session_data)
    ...
    async def update_session(self, session_id: str, data: Dict[str, Any]) -> None:
        ...
        if ttl > 0:
            await self.redis.setex(f"session:{session_id}", ttl, data)
...
class SecurityManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "[SECURITY] Initializing Redis client with URL: %s", self.config.redis_url
            )
            ...
            self.logger.error(
                "[SECURITY] Failed to initialize security services: %s", str(e), exc_info=True
            )
            self._initialized = False
            raise HTTPException(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    self.logger.warning(
                        f"[SECURITY] Health check attempt {attempt + 1} failed, retrying..."
                    )
                    await asyncio.sleep(retry_delay)

        raise ConnectionError(
            f"Redis health check failed after {max_retries} attempts: {str(last_error)}"
        )
    ...
    async def create_session(
        ...
        try:
            ...
            await self.redis_client.set(
                f"refresh_token:{refresh_token}", user_id, expire=2592000  # 30 days
            )
    ...
    def _generate_token(self, user_id: str, is_refresh: bool = False) -> str:
        ...
        payload = {"sub": user_id, "exp": expiration, "type": "refresh" if is_refresh else "access"}
    ...
    async def create_tokens(self, user_id: str) -> Tuple[str, str]:
        ...
        try:
            ...
            await self._redis_client.setex(
                f"refresh_token:{refresh_token}", timedelta(days=30).total_seconds(), user_id
            )
    ...
    async def create_verification_token(self, user_id: str) -> str:
        ...
        await self.redis_client.setex(
            f"verify:{token}", timedelta(hours=24).total_seconds(), user_id
        )
