...
class CodeValidator:
    ...
    async def validate_implementation(self, code: str, task: Task) -> Dict[str, Any]:
        ...
        with self.metrics_collector.track_time("validation_duration"):
            ...
            # Record metrics
            self.metrics_collector.record_counter("validation_errors", len(results["errors"]))
            self.metrics_collector.record_counter("validation_warnings", len(results["warnings"]))
