...
from cahoots_events.models.events import Event, EventPriority, EventStatus
...
async def health_check():
    ...
    return {"status": "healthy", "service": "context-manager", "version": "0.1.0"}
...
async def get_context(project_id: UUID):
    ...
    try:
        ...
        return {"context": context, "version_vector": vector.agent_versions}
...
async def handle_context_event(event: Event):
    ...
    async with project_context(str(event.project_id)) as ctx:
        await context_service.apply_event_to_context(ctx.context, event.data)
