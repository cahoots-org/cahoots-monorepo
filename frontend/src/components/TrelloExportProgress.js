...
const TrelloExportProgress = ({ 
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${wsProtocol}//${wsHost}/ws/trello-export/${exportId}`);

    ws.onopen = () => {
      console.log('WebSocket connected for Trello export progress');
    ...
    ws.onmessage = (event) => {
      try {
        ...
        } else if (data.type === 'trello.export.progress') {
        ...
        } else if (data.type === 'trello.export.success') {
        ...
        } else if (data.type === 'trello.export.error') {
          setHasError(true);
  ...
  const getStepDescription = (step) => {
    switch (step) {
      case 'board_creation':
        return 'Creating Trello Board';
      case 'list_creation':
        return 'Creating Lists';
      case 'card_creation':
        return 'Creating Cards';
      case 'label_creation':
        return 'Creating Labels';
      case 'completion':
        ...
        <div className="p-6">
          <div className="space-y-6">
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                          Created {resultData.cards_created} cards in board "{resultData.board_name}"
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex justify-end space-x-3">
              {isComplete && resultData?.board_url && (
                <a
                  href={resultData.board_url}
                  target="_blank"
...
export default TrelloExportProgress;