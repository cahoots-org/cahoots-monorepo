...
const EventModelTab = ({ task, taskTree }) => {
  ...
  chapters.forEach(chapter => {
    (chapter.slices || []).forEach(sliceRef => {
      // Look up full command/event/read_model objects (case-insensitive matching)
      const normalizeForMatch = (str) => (str || '').toLowerCase().replace(/[_\s-]/g, '');
      const cmd = commands.find(c =>
        normalizeForMatch(c.name) === normalizeForMatch(sliceRef.command)
      );
      ...
      const evts = eventNames.map(name =>
        events.find(e => normalizeForMatch(e.name) === normalizeForMatch(name))
      ).filter(Boolean);
      const rm = readModels.find(r =>
        normalizeForMatch(r.name) === normalizeForMatch(sliceRef.read_model)
      );

      // Find wireframe for this slice
      const sliceName = sliceRef.command || sliceRef.read_model || sliceRef.name;
      const wireframe = wireframes.find(w => normalizeForMatch(w.slice) === normalizeForMatch(sliceName));

      // Include ALL slices from chapters - they were generated by the LLM
      // Even if we can't find matching command/read_model, show the slice
      const sliceType = sliceRef.type || (sliceRef.command ? 'state_change' : sliceRef.read_model ? 'state_view' : 'unknown');

      slices.push({
        name: sliceRef.command || sliceRef.read_model || sliceRef.automation_name || sliceRef.name || 'Unnamed Slice',
        description: cmd?.description || rm?.description || sliceRef.description || '',
        chapter: chapter.name,
        swimlane: cmd?.swimlane || rm?.swimlane || evts[0]?.swimlane,
        command: cmd || (sliceRef.command ? { name: sliceRef.command } : {}),
        event: evts[0] || {},
        events: evts,
        read_model: rm || (sliceRef.read_model ? { name: sliceRef.read_model } : {}),
