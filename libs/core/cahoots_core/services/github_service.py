...
class GitHubService:
    ...
    def create_pull_request(self, repo_name: str, branch: str, title: str, body: str) -> str:
        ...
        repo = self.github.get_repo(f"{self.github.get_user().login}/{repo_name}")
        pr = repo.create_pull(
            title=title, body=body, head=branch, base="main", maintainer_can_modify=True
        )
        return pr.html_url
...
"""
    ...
    def merge_pull_request(self, repo_name: str, pr_number: int) -> bool:
        ...
        try:
            ...
            merge_result = pr.merge(
                commit_title=f"feat: {pr.title}", commit_message=pr.body, merge_method="squash"
            )
    ...
    async def post_review_comments(
        self, pr_number: int, comments: List[Dict[str, Any]], approved: bool, review_message: str
    ) -> None:
        ...
        try:
            ...
            pr.create_review(body=review_message, event=event, comments=review_comments)
    ...
    def clone_repository(self, repo_url: str) -> str:
        ...
        try:
            ...
            auth_url = repo_url.replace(
                "https://", f"https://robmillersoftware:{self.github_api_key}@"
            )
    ...
    async def update_pr_status(self, pr_number: int, status: str) -> None:
        ...
        try:
            ...
            raise ServiceError(
                message=f"Failed to update PR status: {e}", category=ErrorCategory.API
            )
    ...
    async def get_repository_info(self, repo_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {"clone_url": repo.clone_url, "default_branch": repo.default_branch}
        except Exception as e:
