...
class TrelloConfig(Config):
    ...
    def from_env(cls, **defaults) -> "TrelloConfig":
        ...
        try:
            ...
            raise ConfigurationError(
                message="Invalid service configuration", details={"validation_errors": str(e)}
            )
    ...
    def validate_api_key(cls, v: Optional[str]) -> str:
        ...
        if not v:
            raise ConfigurationError(message="Trello API key is required", field="api_key")
        return v
    ...
    def validate_api_token(cls, v: Optional[str]) -> str:
        ...
        if not v:
            raise ConfigurationError(message="Trello API token is required", field="api_token")
        return v
