...
class TrelloClient(AsyncContextManager):
    ...
    async def __aenter__(self):
        """Enter async context."""
        self.session = aiohttp.ClientSession(timeout=ClientTimeout(total=self.config.timeout))
        return self
    ...
    async def request(self, method: str, path: str, **kwargs) -> Any:
        ...
        params.update({"key": self.config.api_key, "token": self.config.api_token})
    ...
    async def create_webhook(self, callback_url: str, id_model: str, **kwargs) -> Dict[str, Any]:
        ...
        """
        data = {"callbackURL": callback_url, "idModel": id_model, **kwargs}
