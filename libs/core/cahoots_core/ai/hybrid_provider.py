...
REQUESTS_TOTAL = Counter(
    "ai_requests_total", "Total AI requests", ["provider", "model", "task_type"]
)
...
REQUEST_DURATION = Histogram(
    "ai_request_duration_seconds", "AI request duration in seconds", ["provider", "model"]
)
ERROR_TOTAL = Counter("ai_errors_total", "Total AI errors", ["provider", "model", "error_type"])
...
class HybridProvider(AIProvider):
    ...
    def __init__(
        ...
        if self.anthropic_api_key:
            ...
            logger.warning(
                "Anthropic API key not provided. Complex reasoning tasks will fall back to Mistral."
            )
    ...
    async def generate_response(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            TOKENS_TOTAL.labels(provider=provider, model=model, direction="output").inc(
                output_tokens
            )
    ...
    async def generate_embeddings(
        self, texts: List[str], *, model: Optional[str] = "mistral-embed", **kwargs: Any
    ) -> List[List[float]]:
        ...
        try:
            embeddings = []
            for text in texts:
                response = await self.mistral_client.embeddings(model=model, input=text)
            ...
            TOKENS_TOTAL.labels(provider="mistral", model=model, direction="input").inc(
                total_tokens
            )
