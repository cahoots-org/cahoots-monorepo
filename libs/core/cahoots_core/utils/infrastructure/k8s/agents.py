...
class AgentDeployment:
    ...
    def _get_agent_env(self, agent_type: str, config: Dict) -> List[Dict]:
        ...
        return [
            ...
            {"name": "REDIS_NAMESPACE", "value": f"project:{self.project_id}"},
            {"name": "DB_SCHEMA", "value": f"project_{self.project_id}"},
    ...
    async def scale_agent(self, agent_type: str, replicas: int):
        ...
        await self.k8s_client.scale_deployment(
            name=deployment_name, namespace=self.namespace, replicas=replicas
        )
    ...
    async def delete_agent(self, agent_type: str):
        ...
        """
        deployment_name = f"{agent_type}-{self.project_id}"
        await self.k8s_client.delete_deployment(name=deployment_name, namespace=self.namespace)
