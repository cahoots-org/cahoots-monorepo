...
class ServiceConfig(BaseModel):
    ...
    github_api_key: Optional[str] = Field(
        default=None, description="GitHub API key for authentication"
    )
    workspace_dir: str = Field(
        default_factory=lambda: tempfile.gettempdir(), description="Workspace directory for service"
    )
...
class SecurityConfig(BaseModel):
    ...
    password_reset_expire_minutes: int = Field(
        default=15, description="Password reset token expiration in minutes"
    )
