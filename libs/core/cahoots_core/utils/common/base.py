...
def rate_limit(
    ...
    Args:
        ...
        raise_on_limit: Whether to raise exception when limit is hit
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            if len(timestamps) >= calls:
                if raise_on_limit:
                    raise ValidationError(f"Rate limit exceeded: {calls} calls per {period} seconds")
                return None
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            if ttl is not None:
                cache_valid = (
                    ...
                    now - timestamps[key] < ttl
                )
            else:
                cache_valid = key in cache
