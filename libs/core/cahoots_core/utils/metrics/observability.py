...
class ObservabilityManager:
    ...
    def __init__(
        ...
        self.performance = performance or PerformanceAnalyzer(service_name, metrics=self.metrics)
    ...
    def end_trace(
        self, trace_ctx: TraceContext, status: str = "OK", error: Optional[Exception] = None
    ):
        """End a trace.
        ...
        trace_ctx.duration = (trace_ctx.end_time - trace_ctx.start_time).total_seconds() * 1000
        ...
        if error:
            logger.error(f"{log_msg} with error: {str(error)}\n" f"{traceback.format_exc()}")
        else:
            logger.info(log_msg)

    def add_trace_event(
        self, trace_ctx: TraceContext, event_name: str, attributes: Optional[Dict[str, Any]] = None
    ):
        ...
        """
        event = {"name": event_name, "timestamp": datetime.utcnow(), "attributes": attributes or {}}
    ...
    def get_completed_traces(self, window: Optional[timedelta] = None) -> List[TraceContext]:
    ...
    def get_failed_traces(self, window: Optional[timedelta] = None) -> List[TraceContext]:
