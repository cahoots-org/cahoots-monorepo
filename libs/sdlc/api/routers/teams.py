...
async def get_team(team_id: UUID, request: Request):
    """Get team details"""
    view = request.state.view_store.get_view(team_id, TeamView)
    if not view:
...
async def add_team_member(team_id: UUID, member: TeamMemberAdd, request: Request):
    """Add member to team"""
    try:
        events = request.state.organization_handler.handle_add_team_member(
            team_id=team_id, member_id=member.member_id, role=member.role
        )
...
async def remove_team_member(team_id: UUID, member_id: UUID, request: Request):
...
async def update_team_member_role(
    team_id: UUID, member_id: UUID, update: TeamMemberUpdate, request: Request
):
    """Update team member role"""
    try:
        events = request.state.organization_handler.handle_update_team_member_role(
            team_id=team_id, member_id=member_id, new_role=update.new_role, reason=update.reason
        )
...
async def transfer_team_leadership(team_id: UUID, new_lead_id: UUID, request: Request):
    """Transfer team leadership"""
    try:
        events = request.state.organization_handler.handle_transfer_team_leadership(
            team_id=team_id, new_lead_id=new_lead_id
        )
...
async def archive_team(team_id: UUID, archive: TeamArchive, request: Request):
    """Archive team"""
    try:
        events = request.state.organization_handler.handle_archive_team(
            team_id=team_id, reason=archive.reason
        )
