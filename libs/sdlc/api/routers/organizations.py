...
async def get_organization(organization_id: UUID, request: Request):
    ...
    view = request.state.view_store.get_view(organization_id, OrganizationDetailsView)
    if not view:
...
async def update_organization_name(
    organization_id: UUID, update: OrganizationUpdate, request: Request
):
    """Update organization name"""
    try:
        events = request.state.organization_handler.handle_update_name(
            organization_id=organization_id, new_name=update.name, reason=update.reason
        )
...
async def add_member(organization_id: UUID, member: MemberAdd, request: Request):
    """Add member to organization"""
    try:
        events = request.state.organization_handler.handle_add_member(
            organization_id=organization_id, user_id=member.user_id, role=member.role
        )
...
async def remove_member(organization_id: UUID, user_id: UUID, request: Request):
...
async def update_member_role(
    organization_id: UUID, user_id: UUID, update: MemberUpdate, request: Request
):
    """Update member role"""
    try:
...
async def list_members(organization_id: UUID, request: Request):
    ...
    view = request.state.view_store.get_view(organization_id, OrganizationMembersView)
    if not view:
        raise HTTPException(status_code=404, detail="Organization not found")
...
async def get_audit_log(organization_id: UUID, request: Request):
    ...
    view = request.state.view_store.get_view(organization_id, OrganizationAuditLogView)
    if not view:
        raise HTTPException(status_code=404, detail="Organization not found")
