...
class WebhookManager:
    ...
    async def deliver_webhook(
        ...
        background_tasks.add_task(self._deliver_webhook, subscription, event_type, payload)

    async def _deliver_webhook(
        self, subscription: WebhookSubscription, event_type: str, payload: Dict
    ):
        ...
        try:
            ...
            headers = {
                ...
                "X-Webhook-Signature": self._generate_signature(payload, subscription.secret),
            }

            # Make the request
            response = await self.http_client.post(
                str(subscription.url), json=payload, headers=headers, timeout=10.0
            )
...
async def create_subscription(subscription: WebhookSubscription, request: Request):
...
async def delete_subscription(subscription_id: UUID, request: Request):
