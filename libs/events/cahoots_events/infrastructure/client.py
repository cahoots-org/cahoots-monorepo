...
class EventClient:
    ...
    async def publish(self, channel: str, message: Dict[str, Any], retry_count: int = 0) -> bool:
    ...
    async def subscribe(
        self, channel: str, handler: Callable[[Dict[str, Any]], Any], pattern: bool = False
    ) -> None:
                ...
                if message:
                    ...
                    for handler in self._handlers[channel]:
                        try:
                            ...
                            logger.error(f"Handler error for channel {channel}: {str(e)}")
