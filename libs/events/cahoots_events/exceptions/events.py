...
class EventError(CahootsError):
    ...
    def __init__(
        ...
        *args: Any,
        ...
        """
        super().__init__(
            message, *args, severity=severity, category=category, details=details, cause=cause
        )
...
class EventSizeLimitExceeded(EventError):
    ...
    def __init__(
        self, message: str, size: int, limit: int, details: Optional[Dict[str, Any]] = None
    ):
        ...
        if details is None:
            details = {}
        details.update({"size": size, "limit": limit, "exceeded_by": size - limit})
        super().__init__(
            message=message,
            details=details,
