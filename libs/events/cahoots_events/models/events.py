...
class ContextEventModel(Base):
    ...
    project_id = Column(
        String, ForeignKey("projects.id"), nullable=False, index=True, doc="Project identifier"
    )
    event_type = Column(
        String,
        nullable=False,
        doc="Type of context event (code_change, architectural_decision, standard_update)",
    )
    ...
    version_vector = Column(
        JSON, nullable=False, default={"master": 0}, doc="Version vector for causal ordering"
    )
...
class ContextEvent(BaseModel):
    ...
    version_vector: VersionVector = Field(
        default_factory=VersionVector.new, description="Version vector for event"
    )
